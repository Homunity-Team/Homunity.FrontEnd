...
function initUpdateMap(lat, lng) {
    ...
    updateMap.on("click", async (e) => { if (!updatePickingMode) return; await setUpdateLocation(e.latlng.lat, e.latlng.lng); setUpdatePickingMode(false); });
}
        ...
        <div class="upload-wrapper w-100">
            <div class="row g-3">
                <div class="col-lg-9 col-12 border-end-divider">
                    <div class="mb-3"><button type="button" id="addImageBtn" class="add-btn-yellow" onclick="document.getElementById('newImagesInput').click()">+ Add Image</button>
                    <input type="file" id="newImagesInput" multiple accept="image/*" class="d-none" onchange="previewNewImages(this)"></div>
                    <div id="imagesGrid" class="images-grid-layout"></div>
                </div>
                <div class="col-lg-3 col-12 ps-lg-4">
                    <div class="mb-3"><button type="button" id="addVideoBtn" class="add-btn-yellow w-100" onclick="document.getElementById('newVideoInput').click()">+ Add Video</button>
                    <input type="file" id="newVideoInput" accept="video/*" class="d-none" onchange="previewNewVideo(this)"></div>
...
function createMediaCard(url, type, isNew = false, fileName = "") {
    ...
    const mediaContent = type === "image" ? `<div class="thumb-img" style="background-image:url('${url}')"></div>` : `<div class="thumb-img video-preview-wrapper"><video src="${url}" controls class="w-100 h-100 rounded-3"></video></div>`;
...
document.getElementById("updatePropertyForm")?.addEventListener("submit", async function (e) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            fetchProperties();
            fetchOwnerStats();
        } else { console.error("Update failed:", resData); alert("Failed: " + (resData.message || "Check your inputs.")); }
    } catch (err) { console.error(err); alert("Connection error."); }
    finally { hideLoader(); }
});
...
function initDetailsMap(lat, lng, address, uniName, distanceKm) {
    ...
    if (!lat || !lng || isNaN(latF) || isNaN(lngF)) {
        mapEl.style.display = "none"; noLocEl.style.display = "flex"; mapInfoEl.classList.add("d-none"); uniInfoEl.classList.add("d-none");
        return;
    }
...
async function showPropertyDetails(prop) {
    ...
    thumbsRow.innerHTML = "";
    if (images.length === 0) {
        grid.innerHTML = `<div class="no-image-placeholder">No images available</div>`;
    } else {
        images.forEach((img, idx) => {
            const imgDiv = document.createElement("div");
            imgDiv.className = "gallery-main-img";
            imgDiv.style.backgroundImage = `url('${img.imageUrl}')`;
            imgDiv.style.display = idx === 0 ? "block" : "none";
            imgDiv.id = `galleryImg_${idx}`;
            ...
            thumb.addEventListener("click", () => {
                document.querySelectorAll(".gallery-main-img").forEach(el => el.style.display = "none");
                document.getElementById(`galleryImg_${idx}`).style.display = "block";
    ...
    try {
        ...
        if (bookings.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="3" class="text-center">No bookings yet.</td></tr>';
        } else {
    ...
    } catch (err) { console.error(err); tableBody.innerHTML = '<tr><td colspan="3" class="text-danger">Error loading bookings.</td></tr>'; }
...
window.handleBookingAction = async (bookingId, action, propertyId) => {
    ...
    try {
        ...
        } else { alert("Error updating booking."); }
    } catch (err) { console.error(err); alert("Connection error."); }
};
...
document.getElementById("onePBtnDelete").onclick = () => {
    openDeleteModal(document.getElementById("idPropirtie").value, document.getElementById("onePTitle").textContent,
        document.getElementById("onePAddress").textContent, document.getElementById("onePMainImg")?.src || "");
};
...
document.getElementById("confirmDeleteBtn").onclick = async function () {
    ...
    try {
        ...
        } else { const err = await res.json(); alert("Error: " + (err.message || "Could not delete.")); }
...
window.handleAction = async (bookingId, type) => {
    ...
    try {
        const res = await fetch(url, { method: "PUT", headers: { accept: "*/*" } });
        if (res.ok) {
            Swal.fire({ icon: "success", title: type === "accept" ? "Confirmed!" : "Rejected!", timer: 1500, showConfirmButton: false });
            setTimeout(() => fetchBookings(), 1600);
        } else alert("Error: " + ((await res.json()).message || "Try again"));
    } catch (e) { console.error(e); alert("Connection error"); }
};
...
if (!document.getElementById("dynamicGalleryStyles")) {
    ...
    style.textContent = `
        .gallery-grid { display: flex; justify-content: center; align-items: center; background: #f4f4f4; border-radius: 16px; min-height: 350px; }
        .gallery-main-img { width: 100%; height: 350px; background-size: contain; background-repeat: no-repeat; background-position: center; border-radius: 12px; }
        .thumbnails-row { display: flex; justify-content: center; gap: 12px; margin-top: 20px; flex-wrap: wrap; }
        ...
        .gallery-thumb.active-thumb { border-color: #efb81e; transform: scale(1.05); }
        .btn-accept-sm, .btn-reject-sm { padding: 4px 12px; font-size: 12px; border-radius: 20px; margin: 2px; border: none; }
        .btn-accept-sm { background: #efb81e; color: #212e43; }
        .btn-reject-sm { background: #212e43; color: #efb81e; }
        @media (max-width: 768px) { .gallery-main-img { height: 250px; } .gallery-thumb { width: 50px; height: 50px; } }
    `;
