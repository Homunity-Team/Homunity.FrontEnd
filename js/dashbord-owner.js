...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (phoneInput)
    phoneInput.addEventListener("input", () => {
      document.getElementById("phoneError").innerText = !phoneInput.value.trim()
        ? ""
  ...
  if (firstInput)
    firstInput.addEventListener("input", () => {
      ...
      document.getElementById("firstNameError").innerText = !v
        ? ""
  ...
  if (lastInput)
    lastInput.addEventListener("input", () => {
      ...
      document.getElementById("lastNameError").innerText = !v
        ? ""
        : v.length < 2 || v.length > 10
...
function findNearest(lat, lng) {
  ...
  allUniversities.forEach((u) => {
    const d = parseFloat(calcDist(lat, lng, u.latitude, u.longitude));
    if (d < minDist) {
      minDist = d;
      nearest = u;
    }
...
async function nominatimSearch(q) {
  try {
    const r = await fetch(
      `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&countrycodes=eg&limit=1`,
    );
    const d = await r.json();
...
function setAddPick(active) {
  ...
  const banner = document.getElementById("addPickingBanner"),
    btn = document.getElementById("addManualBtn"),
    wrap = document.getElementById("addMapContainer");
  if (active) {
    banner && banner.classList.add("show");
    btn && btn.classList.add("active");
    wrap && wrap.classList.add("map-picking-mode");
  } else {
    banner && banner.classList.remove("show");
    btn && btn.classList.remove("active");
    wrap && wrap.classList.remove("map-picking-mode");
  }
}
async function setAddLoc(lat, lng) {
  ...
  if (nearest)
    addMarker
      .bindPopup(
        `<b>${address.split(",")[0]}</b><br><small>Near ${nearest.university.name} (${nearest.distance} km)</small>`,
      )
      .openPopup();
}
...
document.getElementById("addGpsBtn").addEventListener("click", function () {
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      await setAddLoc(pos.coords.latitude, pos.coords.longitude);
...
document.getElementById("addManualBtn").addEventListener("click", () => {
  initAddMap();
  addPickingMode ? setAddPick(false) : setAddPick(true);
});
document
  .getElementById("addMapSearchBtn")
  .addEventListener("click", async () => {
    const q = document.getElementById("addMapSearch").value.trim();
    if (!q) return;
    initAddMap();
    await fetchUniversities();
    const r = await nominatimSearch(q);
    if (r) await setAddLoc(parseFloat(r.lat), parseFloat(r.lon));
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const title = get("titleAdd"),
      price = get("priceAdd"),
      rooms = get("roomsAdd"),
      images = get("images"),
      video = get("video");
    const lat = get("addLat").value,
      lng = get("addLng").value,
      address = get("addAddress").value,
      universityId = get("addUniversityId").value;
    let isValid = true;
    ...
    if (title.value.length < 5) {
      toggleErr("titleError", true);
      isValid = false;
    } else toggleErr("titleError", false);
    if (!price.value || parseFloat(price.value) < 100) {
      toggleErr("priceError", true);
      isValid = false;
    } else toggleErr("priceError", false);
    if (
      !rooms.value ||
      parseInt(rooms.value) < 1 ||
      parseInt(rooms.value) > 10
    ) {
      toggleErr("roomsError", true);
      isValid = false;
    } else toggleErr("roomsError", false);
    if (!lat || !lng) {
      get("addLocationError").style.display = "block";
      isValid = false;
    } else get("addLocationError").style.display = "none";
    ...
    try {
      ...
      fd.append(
        "PropertyType",
        get("apartmentApp").checked ? "Apartment" : "Room",
      );
      ...
      const res = await fetch(`${API_BASE}/Properties/CreateFullProperty`, {
        method: "POST",
        body: fd,
      });
      if (res.ok) {
        ...
        ["addLat", "addLng", "addAddress", "addUniversityId"].forEach((id) => {
          const el = get(id);
          if (el) el.value = "";
        });
        if (addMarker) {
...
function initUpdateMap(lat, lng) {
  ...
  const center =
      lat && lng ? [parseFloat(lat), parseFloat(lng)] : [30.04, 31.23],
    zoom = lat && lng ? 15 : 11;
  ...
  if (lat && lng)
    updateMarker = L.marker([parseFloat(lat), parseFloat(lng)], {
      icon: goldIcon(),
    }).addTo(updateMap);
  updateMap.on("click", async (e) => {
    if (!updatePickingMode) return;
    await setUpdateLoc(e.latlng.lat, e.latlng.lng);
    setUpdatePick(false);
  });
...
function setUpdatePick(active) {
  ...
  const banner = document.getElementById("updatePickingBanner"),
    btn = document.getElementById("updateManualBtn"),
    wrap = document.getElementById("updateMapContainer");
  if (active) {
    banner && banner.classList.add("show");
    btn && btn.classList.add("active");
    wrap && wrap.classList.add("map-picking-mode");
...
document.getElementById("updateGpsBtn").addEventListener("click", function () {
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      await fetchUniversities();
      await setUpdateLoc(pos.coords.latitude, pos.coords.longitude);
      btn.innerHTML = `<i class="fa-solid fa-location-crosshairs"></i> Use My Location`;
      btn.disabled = false;
    },
    (err) => {
...
document.getElementById("updateManualBtn").addEventListener("click", () => {
  updatePickingMode ? setUpdatePick(false) : setUpdatePick(true);
});
document
  .getElementById("updateMapSearchBtn")
  .addEventListener("click", async () => {
    const q = document.getElementById("updateMapSearch").value.trim();
    if (!q) return;
    await fetchUniversities();
    const r = await nominatimSearch(q);
    if (r) await setUpdateLoc(parseFloat(r.lat), parseFloat(r.lon));
    else alert("Not found.");
  });
document.getElementById("updateMapSearch").addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    e.preventDefault();
    document.getElementById("updateMapSearchBtn").click();
  }
});
...
async function fetchOwnerStats() {
  ...
  try {
    ...
    const props = pData.properties || [],
      books = bData.bookings || [];
    const el = (id) => document.getElementById(id);
    if (el("total-props")) el("total-props").innerText = props.length;
    if (el("approved-count"))
      el("approved-count").innerText = props.filter(
        (p) => p.propertyStatusID === 2,
      ).length;
    if (el("rejected-count"))
      el("rejected-count").innerText = props.filter(
        (p) => p.propertyStatusID === 3,
      ).length;
    if (el("pending-booking"))
      el("pending-booking").innerText = books.filter(
        (b) => b.statusName === "In-Process",
      ).length;
    if (el("booked-count"))
      el("booked-count").innerText = books.filter(
...
function openUpdateSection(prop) {
  ...
  if (prop.propertyType && prop.propertyType.toLowerCase() === "apartment")
    get("apartmentUpdate").checked = true;
  else get("roomUpdate").checked = true;
  ["wifiUpdate", "parkingUpdate", "gymUpdate", "acUpdate"].forEach(
    (id) => (get(id).checked = false),
  );
  if (prop.services)
  ...
  const existingLat = loc.latitude || null,
    existingLng = loc.longitude || null;
  const existingAddr =
    prop.fullAddress ||
    loc.address ||
    [loc.street, loc.area, loc.city].filter(Boolean).join(", ") ||
    "";
  get("updateLat").value = existingLat || "";
  get("updateLng").value = existingLng || "";
  get("updateAddress").value = existingAddr;
  if (existingLat && existingLng) {
    get("updateSelectedAddress").textContent =
      existingAddr || `${existingLat},${existingLng}`;
...
function previewNewImages(input) {
  ...
  Array.from(input.files).forEach((f) => {
    ...
    grid.appendChild(
      createMediaCard(URL.createObjectURL(f), "image", true, f.name),
    );
  });
  checkImageLimit();
  input.value = "";
...
function removeMediaItem(btn, type) {
  ...
  if (type === "video") {
    newVideoFile = null;
    const b = document.getElementById("addVideoBtn");
...
document
  ...
  ?.addEventListener("submit", async function (e) {
    e.preventDefault();
    let isValid = true;
    function showErr(input, id, cond, msg) {
    ...
    const get = (id) => document.getElementById(id);
    const title = get("titleUpdate"),
      price = get("priceUpdate"),
      rooms = get("roomsUpdate");
    showErr(
      title,
      "titleErrorUpdate",
    ...
    try {
      ...
      const uLat = get("updateLat").value,
        ...
        uAddr = get("updateAddress").value,
        uUniId = get("updateUniversityId").value;
      const fd = new FormData();
      ...
      fd.append("Latitude", parseFloat(uLat || 0));
      fd.append("Longitude", parseFloat(uLng || 0));
      fd.append("Address", uAddr);
      fd.append("UniversityId", parseInt(uUniId || 0));
      newPropertyFiles.forEach((f) => fd.append("NewImages", f));
      if (newVideoFile) {
        fd.append("NewVideo", newVideoFile);
        fd.append("DeleteVideo", "false");
      } else
        fd.append(
          "DeleteVideo",
          (
      ...
      [
        ...
        if (get(s.id)?.checked) fd.append("Services", s.val);
      });
      const res = await fetch(`${API_BASE}/Properties/UpdateFullProperty`, {
        method: "PUT",
        body: fd,
      });
...
async function fetchProperties() {
  ...
  if (!ownerId) {
    container.innerHTML =
      "<p class='text-center py-5 text-warning'>Please login.</p>";
    return;
  }
  showLoader();
  try {
    const res = await fetch(
      `${API_BASE}/Properties/GetByOwnerV2?ownerId=${ownerId}`,
    );
    const data = await res.json();
    container.innerHTML = "";
    ...
    properties.forEach((prop) => {
      ...
      if (!addrDisplay || addrDisplay.includes("Default")) {
        const loc = prop.location || {};
        addrDisplay =
          loc.address ||
          [loc.street, loc.area, loc.city].filter(Boolean).join(", ");
    ...
    console.error(err);
    container.innerHTML =
      "<p class='text-center text-danger py-5'>Error loading properties.</p>";
  } finally {
    hideLoader();
  }
}
...
function initDetailsMap(lat, lng, address, uniName, distKm) {
  ...
  const mapEl = document.getElementById("detailsMap"),
    noLocEl = document.getElementById("detailsMapNoLocation"),
    mapInfoEl = document.getElementById("detailsMapInfo"),
    uniInfoEl = document.getElementById("detailsUniInfo");
  const latF = lat ? parseFloat(lat) : NaN,
    lngF = lng ? parseFloat(lng) : NaN;
  ...
  detailsMap = L.map("detailsMap", {
    zoomControl: true,
    scrollWheelZoom: false,
    preferCanvas: true,
  }).setView([latF, lngF], 15);
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    attribution: "© OpenStreetMap",
    maxZoom: 18,
  }).addTo(detailsMap);
  ...
  if (address) {
    marker
      .bindPopup(`<b style="color:#212e43">${address.split(",")[0]}</b>`)
      .openPopup();
    document.getElementById("detailsMapAddress").textContent = address;
    mapInfoEl.classList.remove("d-none");
  }
  if (uniName) {
    document.getElementById("detailsUniText").textContent =
      `Nearest University: ${uniName}${distKm ? ` — ${distKm} km away` : ""}`;
...
async function showPropertyDetails(prop) {
  ...
  window._gIdx = 0;
  window._gTotal = images.length;
  window.slideGallery = (dir) => {
    if (window._gTotal <= 1) return;
    window._gIdx = (window._gIdx + dir + window._gTotal) % window._gTotal;
    goToSlide(window._gIdx);
  };
  window.goToSlide = (idx) => {
    ...
    images.forEach((_, i) => {
      const dot = document.getElementById(`gDot_${i}`);
      if (dot)
        dot.style.background = i === idx ? "#efb81e" : "rgba(255,255,255,0.5)";
      const th = document.getElementById(`gThumb_${i}`);
      if (th) th.style.borderColor = i === idx ? "#efb81e" : "transparent";
    });
  };
  ...
  document.getElementById("onePPrice").innerHTML =
    `<i class="fas fa-diamond"></i> Price <span style="color:#efb81e;font-weight:700;">$${prop.price} / month</span>`;
  document.getElementById("onePRooms").innerHTML =
    `<i class="fas fa-diamond"></i> ${prop.rooms} rooms`;

  const amenitiesList = document.getElementById("onePAmenitiesList");
  ...
  if (prop.services && prop.services.length > 0) {
    prop.services.forEach((s) => {
      ...
      li.style.cssText =
        "display:flex;align-items:center;gap:12px;margin-bottom:12px;font-size:1rem;font-weight:600;color:#212e43;";
      li.innerHTML = `<i class="${icon}" style="color:#efb81e;font-size:1.1rem;width:20px;text-align:center;"></i>${escapeHtml(s.name)}`;
      amenitiesList.appendChild(li);
    });
  } else
    amenitiesList.innerHTML = `<li style="color:#94a3b8;">No services listed</li>`;

  const lat = loc.latitude || null,
    lng = loc.longitude || null;
  const uniName = loc.university ? loc.university.name : null,
    distKm = loc.university ? loc.university.distance_km : null;
  ...
  try {
    ...
    console.error(err);
    tableBody.innerHTML =
      '<tr><td colspan="3" class="text-danger text-center">Error loading bookings.</td></tr>';
  }
...
window.handleBookingAction = async (bookingId, action, propertyId) => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      setTimeout(async () => {
        const r = await fetch(
          `${API_BASE}/Properties/GetByIDV2?id=${propertyId}`,
        );
        const d = await r.json();
        if (d.property) showPropertyDetails(d.property);
      }, 1600);
...
function openDeleteModal(id, title, address, img) {
  ...
  if (img) document.getElementById("deletePropImg").src = img;
...
document.getElementById("confirmDeleteBtn").onclick = async function () {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/Properties/DeleteProperty?id=${propertyIdToDelete}`,
      { method: "DELETE" },
    );
    if (res.ok) {
      Swal.fire({
...
window.handleAction = async (bookingId, type) => {
  const url =
    type === "accept"
      ? `${API_BASE}/Booking/${bookingId}/confirm?OwnerId=${ownerIdBooking}`
      : `${API_BASE}/Booking/${bookingId}/cancel`;
  try {
    const res = await fetch(url, { method: "PUT", headers: { accept: "*/*" } });
    ...
    if (res.ok) {
      Swal.fire({
        icon: "success",
        title: type === "accept" ? "Confirmed!" : "Rejected!",
        timer: 1500,
        showConfirmButton: false,
      });
      setTimeout(() => fetchBookings(), 1600);
    } else
      Swal.fire({
        icon: "error",
        title: "Error",
...
function renderMessagesCards(bookings) {
  ...
  messagesContainer.innerHTML = bookings
    .map(
      (b) =>
        `<div class="msg-card mb-3"><div class="row align-items-center g-2"><div class="col-auto"><div style="width:70px;height:70px;overflow:hidden;border-radius:10px;"><img src="${b.property.imageUrl}" style="width:100%;height:100%;object-fit:cover;"></div></div><div class="col text-start ps-3"><div class="fw-bold text-warning">${escapeHtml(b.property.title)}</div><div class="small text-white">${escapeHtml(b.studentName)}</div><div class="text-white-50" style="font-size:0.7rem;">Status: ${b.statusName}</div></div><div class="col-auto"><span class="status-btn-mock">${b.statusName === "Cancelled" ? "Rejected" : "Booked"}</span></div></div></div>`,
    )
    .join("");
}
